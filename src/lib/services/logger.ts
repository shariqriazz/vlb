...
// Configure transport for target management
const targetTransport = new winston.transports.DailyRotateFile({
  filename: path.join(logsDir, 'targets-%DATE%.log'), // Changed filename
  datePattern: 'YYYY-MM-DD',
  maxSize: '20m',
  maxFiles: '14d',
  format: winston.format.combine(
...
const targetLogger = winston.createLogger({ // Renamed logger
  transports: [
    targetTransport, // Use renamed transport
    new winston.transports.Console({
      format: winston.format.combine(
        winston.format.colorize(),
        winston.format.simple()
...
// Helper function to log target management events
export const logTargetEvent = (event: string, details: any) => { // Renamed function
  targetLogger.info(event, details); // Use renamed logger
};
...
// Update type union and add targetId to options
export const getLogs = async (type: 'requests' | 'errors' | 'targets', options: {
  limit?: number;
  startDate?: string; // YYYY-MM-DD
  endDate?: string;   // YYYY-MM-DD
  search?: string;
  targetId?: string; // Added targetId option
} = {}) => {
  // Destructure targetId from options
  const { limit = 1000, startDate: startDateStr, endDate: endDateStr, search, targetId } = options;
  const logPrefix = `${type}-`; // This will now correctly form 'targets-'
      ...
      try {
        ...
        for (let i = lines.length - 1; i >= 0; i--) {
          ...
          try {
            ...
            // --- Target ID Filtering ---
            // Check if targetId filter is provided and if the log entry matches
            if (targetId && logEntry.targetId !== targetId) {
                continue; // Skip if targetId doesn't match
            }
            // --- End Target ID Filtering ---
